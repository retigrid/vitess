SELECT 
		UNIX_TIMESTAMP(DATE_ADD( '0000-01-01', INTERVAL (CEIL((YEAR(FROM_UNIXTIME(created_ts / 1000000000)) * 12 + MONTH(FROM_UNIXTIME(created_ts / 1000000000)))/ 2) * 2) MONTH )) * 1000000000 DIV 1, 
  	NOW(),
		serial_id,
		group_id,
...
GROUP BY
    serial_id,
		group_id,
		CEIL((YEAR(FROM_UNIXTIME(created_ts / 1000000000)) * 12 + MONTH(FROM_UNIXTIME(created_ts / 1000000000)))/ 2) 
